...
export async function renderUserView() {
    ...
    <main class="dashboard-main">
      ...
      <section class="organizers-section">
        <div class="organizers-container">
          <h2 class="section-title">Theo dõi các giải đấu để nhận thông báo mới nhất từ các nhà tổ chức hàng đầu</h2>
          ...
          <div class="organizers-navigation">
  ...
  // Setup event listeners for user dashboard
  setupUserViewEvents();
...
function setupUserViewEvents() {
  // Setup home navigation
  const homeNavItem = document.querySelector('.home-nav-item[data-home-nav]');
  if (homeNavItem) {
  ...
  // Organizer card click functionality
  document.querySelectorAll('.organizer-card .organizer-image').forEach(card => {
    card.addEventListener('click', function() {
      if (window.router) {
        window.router.navigate('/organizer-detail');
      }
    });
  });
}
...
async function loadUserContent() {
  try {
    // Load carousels with user-specific content
    await Promise.all([
      loadTournamentsCarousel('tournamentContainer', 3),
      loadNewsCarousel('newsContainer', 3),
      loadHighlightsCarousel('highlightsContainer', 1)
    ]);
  } catch (error) {
